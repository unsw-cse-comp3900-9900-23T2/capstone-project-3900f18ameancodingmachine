...
export async function getSearchResults (req, res) {
    try {
        ...
        const cuisineMatch = (await getEateriesByCuisine(cuisine)).results
        const dietMatch = (await getEateriesByDiet(diet)).results
        const stringMatch = (await getEateriesBySearchString(string)).results
...
export async function storeEateryLayoutImgController (req, res) {
    try {
        const result = await storeEateryLayoutImg(req.file.path, req.body.restaurantId)
        return res.status(200).json(result)
    } catch (error) {
        console.log(error)
        return res.status(500).json({
